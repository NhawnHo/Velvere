...
const UserList: React.FC = () => {
  ...
  useEffect(() => {
    const checkAuth = async () => {
      try {
        ...
        const response = await axios.get(`${apiBaseUrl}/api/users/check-session`, {
  ...
  const toggleAdminStatus = async (userId: string, currentStatus: boolean) => {
    try {
      ...
      await axios.patch(`${apiBaseUrl}/api/users/${userId}`, {
                ...
                {users.length > 0 ? (
                  users.map((user) => (
                    <tr key={user._id} className="border-b hover:bg-gray-50">
                      ...
                      <td className="py-3 px-4">{formatDate(user.birthDate)}</td>
                      ...
                      <td className="py-3 px-4">
                        <div className="flex justify-center space-x-3">
                          ...
                          >
                            <i className={`fa-solid ${user.isAdmin ? 'fa-user' : 'fa-user-shield'} text-lg`}></i>
                          </button>
                          <button
