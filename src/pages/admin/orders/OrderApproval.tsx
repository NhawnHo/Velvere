...
export default function OrderApproval() {
  // eslint-disable-next-line
    ...
    const [showApproveAllModal, setShowApproveAllModal] = useState<boolean>(false);
    ...
    const handleApproveOrder = async (orderId: string) => {
        try {
            ...
            if (!response.ok) {
                throw new Error(`Không thể duyệt đơn hàng: ${response.status} ${response.statusText}`);
            }
            ...
            setError(err instanceof Error ? err.message : 'Đã xảy ra lỗi khi duyệt đơn hàng');
        } finally {
            setProcessing(false);
        }
    };
    ...
    const handleCancelOrder = async () => {
        ...
        try {
            setProcessing(true);
            const apiBaseUrl = import.meta.env.VITE_API_BASE_URL || 'http://localhost:3000';
            const response = await fetch(`${apiBaseUrl}/api/orders/${selectedOrder._id}/cancel`, {
                method: 'PUT',
                credentials: 'include',
                headers: {
                    'Content-Type': 'application/json'
            ...
            if (!response.ok) {
                throw new Error(`Không thể hủy đơn hàng: ${response.status} ${response.statusText}`);
            }
        ...
        <div className="container mx-auto p-6">
            ...
            <div className="flex justify-between items-center mb-6">
                <div>
                    ...
                    {!loading && (
                        <p className="text-gray-600 mt-2">
                            Có <span className="font-semibold text-blue-600">{orders.length}</span> đơn hàng đang chờ duyệt
                        </p>
                    )}
                </div>
                ...
                {orders.length > 0 && (
                    <div className="flex space-x-4">
                        ...
                        >
                            {processing ? 'Đang xử lý...' : `Duyệt (${selectedOrders.length})`}
                        </button>
                    </div>
                )}
            </div>
            ...
            {loading ? (
                ...
                <div className="text-center py-16 bg-white rounded-lg shadow-sm border">
                    ...
                    <p className="text-xl text-gray-600">Tất cả đơn hàng đã được duyệt!</p>
                    <p className="text-gray-500 mt-2">Không có đơn hàng nào đang chờ duyệt.</p>
                </div>
            ) : (
                <div className="space-y-6">
                    {orders.map((order) => (
                        <div key={order._id} className="border rounded-lg p-6 shadow-sm bg-white relative">
                            ...
                            <div className="flex justify-between items-center mb-4 ml-8">
                                <h2 className="text-xl font-medium">Đơn hàng #{order.order_id}</h2>
                            ...
                            <div className="grid md:grid-cols-2 gap-6 mb-4 ml-8">
                                <div>
                                    ...
                                    <p><span className="font-medium">Tên:</span> {order.user_name}</p>
                                ...
                                <div>
                                    ...
                                    <p><span className="font-medium">Phương thức thanh toán:</span> {order.payment_method}</p>
                                    ...
                                    {order.items.map((item, index) => (
                                        <div key={index} className="flex items-center p-3 border rounded">
                                            ...
                                            <div className="flex-1">
                                                ...
                                                <p className="mt-1">{item.price.toLocaleString('vi-VN')} đ</p>
                                            </div>
                                        </div>
                                    ))}
                                </div>
            ...
            {showCancelModal && selectedOrder && (
                <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
                    <div className="bg-white rounded-lg p-6 w-full max-w-md">
                        <h3 className="text-xl font-semibold mb-4">Xác nhận hủy đơn hàng</h3>
                        <p className="mb-4">Bạn có chắc chắn muốn hủy đơn hàng #{selectedOrder.order_id}?</p>
            ...
            {showApproveAllModal && (
                <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
                    <div className="bg-white rounded-lg p-6 w-full max-w-md">
                        <h3 className="text-xl font-semibold mb-4">Xác nhận duyệt đơn hàng</h3>
                        <p className="mb-4">
                            Bạn có chắc chắn muốn duyệt <span className="font-semibold">{selectedOrders.length}</span> đơn hàng đã chọn?
                        </p>
                        <p className="text-gray-600 mb-6">
                            Khi duyệt, trạng thái của các đơn hàng sẽ chuyển sang "Đã duyệt" và sẽ được đưa vào quy trình giao hàng.
                        </p>
